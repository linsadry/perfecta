  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",color:C.cream}}>
      <div style={{maxWidth:900,margin:"0 auto",padding:"0 24px"}}>
        <div style={{padding:"24px 0 12px",display:"flex",alignItems:"center",gap:12,borderBottom:`1px solid ${C.border}`,marginBottom:24}}>
          <button onClick={onBack} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer"}}>←</button>
          <div>
            <div style={{fontSize:18,fontFamily:"'Playfair Display',serif",fontWeight:700,color:C.cream}}>Dia Ideal</div>
            <div style={{fontSize:11,color:C.muted}}>Pesos e ações ativas</div>
          </div>
        </div>
        <div style={{background:C.bgCard,borderRadius:14,padding:16,border:`1px solid ${C.border}`,marginBottom:24}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8}}>
            {labels.map((band,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontSize:12,color:C.creamM,width:60,flexShrink:0}}>{band.min}–{band.max===101?"100+":band.max}</span>
                <div style={{flex:1,padding:"5px 8px",borderRadius:8,background:C.bgEl,border:`1px solid ${C.border}`,fontSize:12,color:band.color,fontWeight:500}}>{band.label}</div>
              </div>
            ))}
          </div>
        </div>
...
export default function App() {
  ...
  if(showCfg) return (
    <>
      <style>{GLOBAL_STYLE}</style>
      <ConfigScreen taskConfig={taskConfig} scoreLabels={scoreLabels} onSave={saveConfig} onBack={()=>setShowCfg(false)}/>
    </>
  );
