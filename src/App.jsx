...
export default function App() {
  ...
  const [dayTaskForm,     setDayTaskForm]    = useState({label:"",pillar:"saude",type:"essencial",points:10});
  const key = todayKey();
  const [today,   setToday]  = useState({profile:"consultorio",checks:{},score:0,gcal_detected:null,day_tasks:[],notes:""});
  ...
  const allTasks = buildTasks(taskConfig, [...customTasks, ...(today.day_tasks||[])]);
  const profile  = PROFILES[today.profile]||PROFILES.consultorio;
  const baseIds  = profile.tasks.filter(id=>allTasks.find(t=>t.id===id));
  const dayIds   = (today.day_tasks||[]).map(t=>t.id).filter(id=>allTasks.find(t=>t.id===id));
  const atIds    = [...new Set([...baseIds,...dayIds])];
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(day) setToday({profile:day.profile||"consultorio",checks:day.checks||{},score:day.score||0,gcal_detected:day.gcal_detected||null,day_tasks:day.day_tasks||[],notes:day.notes||""});
  ...
  const onProfile=useCallback(p=>update({profile:p,checks:{},gcal_detected:null,day_tasks:[]}),[update]);

  const onNotes=useCallback(v=>update({notes:v}),[update]);

  const addDayTask=useCallback(task=>{
    const next=[...(today.day_tasks||[]),task];
    update({day_tasks:next});
  },[today.day_tasks,update]);

  const removeDayTask=useCallback(id=>{
    const next=(today.day_tasks||[]).filter(t=>t.id!==id);
    const checks={...today.checks};delete checks[id];
    update({day_tasks:next,checks});
  },[today.day_tasks,today.checks,update]);
  ...
  const handleAddDayTask=()=>{
    if(!dayTaskForm.label.trim())return;
    const id=genId(dayTaskForm.label+"_day");
    addDayTask({id,label:dayTaskForm.label.trim(),pillar:dayTaskForm.pillar,type:dayTaskForm.type,points:Number(dayTaskForm.points),custom:true,dayOnly:true});
    setDayTaskForm({label:"",pillar:"saude",type:"essencial",points:10});
    setShowDayTaskForm(false);
  };

  const inputS={padding:"7px 10px",borderRadius:8,border:`1px solid ${C.border}`,background:C.bgEl,color:C.cream,fontSize:12,outline:"none",width:"100%"};

  const DayExtras=(
    <div style={{marginBottom:14}}>
      {/* Tarefas do dia */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        <div style={{fontSize:10,color:C.amber,letterSpacing:"0.15em",fontWeight:700,textTransform:"uppercase"}}>✦ Extras do dia</div>
        {!showDayTaskForm&&(
          <button onClick={()=>setShowDayTaskForm(true)} style={{fontSize:11,color:C.amber,background:"none",border:`1px solid ${C.amber}60`,borderRadius:8,padding:"3px 10px",cursor:"pointer"}}>
            + tarefa
          </button>
        )}
      </div>
      ...
      {showDayTaskForm&&(
        <div style={{background:C.bgEl,borderRadius:12,padding:12,border:`1px solid ${C.amber}40`,marginBottom:10}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <input style={inputS} placeholder="Nome da tarefa" value={dayTaskForm.label}
              onChange={e=>setDayTaskForm(p=>({...p,label:e.target.value}))}/>
            <select style={{...inputS,cursor:"pointer"}} value={dayTaskForm.pillar}
              onChange={e=>setDayTaskForm(p=>({...p,pillar:e.target.value}))}>
              {PILLARS.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}
            </select>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 60px",gap:8,marginBottom:8}}>
            <select style={{...inputS,cursor:"pointer"}} value={dayTaskForm.type}
              onChange={e=>setDayTaskForm(p=>({...p,type:e.target.value}))}>
              <option value="essencial">Essencial (✓/✗)</option>
            ...
            <div style={{fontSize:10,color:C.muted,display:"flex",alignItems:"center",gap:6}}>
              <span>pts:</span>
              <input type="number" min="1" max="100" style={{...inputS,width:60}} value={dayTaskForm.points}
                onChange={e=>setDayTaskForm(p=>({...p,points:e.target.value}))}/>
            </div>
            <div/>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={handleAddDayTask} style={{flex:1,padding:"7px 0",borderRadius:8,border:"none",background:C.amber,color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer"}}>Adicionar</button>
            <button onClick={()=>setShowDayTaskForm(false)} style={{padding:"7px 12px",borderRadius:8,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:12,cursor:"pointer"}}>✕</button>
          </div>
        </div>
      )}

      {(today.day_tasks||[]).length>0&&(today.day_tasks).map(t=>(
        <div key={t.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
          <div style={{flex:1}}>
            <EssencialCard task={t} value={today.checks[t.id]} onChange={onCheck}/>
          </div>
          <button onClick={()=>removeDayTask(t.id)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:14,flexShrink:0,marginBottom:7}}>✕</button>
        </div>
      ))}

      {/* Anotações do dia */}
      <div style={{marginTop:12}}>
        <div style={{fontSize:10,color:C.muted,letterSpacing:"0.12em",fontWeight:700,textTransform:"uppercase",marginBottom:6}}>Anotações do dia</div>
        <textarea
          placeholder="Algo fora do padrão, observação, como foi o dia…"
          value={today.notes||""}
          onChange={e=>onNotes(e.target.value)}
          rows={3}
          style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${C.border}`,
            background:C.bgCard,color:C.creamM,fontSize:12,outline:"none",resize:"vertical",
  ...
  const TabHist=(
    <>
      ...
      {history.slice(0,14).map(d=>{
        ...
        return (
          <div key={d.date} style={{background:C.bgCard,borderRadius:12,padding:"11px 14px",border:`1px solid ${C.border}`,marginBottom:8}}>
            ...
            {d.notes&&<div style={{fontSize:11,color:C.muted,marginTop:8,paddingTop:8,borderTop:`1px solid ${C.border}`,fontStyle:"italic",lineHeight:1.5}}>{d.notes}</div>}
          </div>
        );
      })}
    </>
