...
function ConfigScreen({ taskConfig, scoreLabels, onSave, onBack }) {
  const [cfg,setCfg]=useState(()=>{
    ...
    TASKS_META.forEach(t=>{
      const def=DEFAULT_TASK_CONFIG[t.id]; const usr=taskConfig[t.id];
        ...
        {PILLARS.map(p=>{
          ...
          return (
            <div key={p.id} style={{background:C.bgCard,borderRadius:14,padding:14,border:`1px solid ${C.border}`,
              ...
              {tasks.map(t=>{
                const tc=cfg[t.id]||{}; const pts=tc.points??DEFAULT_TASK_CONFIG[t.id]?.points; const isArr=Array.isArray(pts);
                return (
