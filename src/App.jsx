...
import {
  ...
  buildTasks, computeScore, pillarScore, scoreMeta, getEarned, getMax, todayKey,
} from "./data.js";
...
function AuthScreen({ onAuth }) {
  ...
  const [mode,setMode]=useState("magic"); const [msg,setMsg]=useState(""); const [load,setLoad]=useState(false);
  const submit=async()=>{
    if(!email)return; setLoad(true); setMsg("");
    if(mode==="magic"){
      const{error}=await sb.signInMagic(email);
      setMsg(error?`Erro: ${error.message||"tente novamente"}`:"✓ Link enviado para seu email!");
    } else {
      const{data,error}=await sb.signIn(email,pass);
      if(error) setMsg(`Erro: ${error.message||"credenciais inválidas"}`);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",
      ...
      <img src={HEADER_IMG} alt="" style={{width:72,height:72,borderRadius:16,objectFit:"cover",
        border:`1px solid ${C.border}`,marginBottom:16,opacity:0.9}}/>
      <div style={{fontSize:28,fontFamily:"'Playfair Display',serif",fontWeight:700,color:C.cream,marginBottom:4}}>Perfecta</div>
      <div style={{fontSize:12,color:C.muted,marginBottom:32}}>Seu dia alinhado começa aqui</div>
      <div style={{width:"100%",maxWidth:340}}>
        <div style={{display:"flex",marginBottom:20,background:C.bgCard,borderRadius:10,padding:3}}>
          {[["magic","Magic Link"],["password","Senha"]].map(([k,l])=>(
            <button key={k} onClick={()=>setMode(k)} style={{flex:1,padding:"8px 0",borderRadius:8,fontSize:12,
              border:"none",background:mode===k?C.rust:"transparent",color:mode===k?"#fff":C.muted,cursor:"pointer"}}>{l}</button>
          ))}
        </div>
        <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="seu@email.com" type="email"
          ...
          style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`1px solid ${C.border}`,
            background:C.bgEl,color:C.cream,fontSize:13,outline:"none",marginBottom:10,fontFamily:"inherit"}}/>
        {mode==="password"&&<input value={pass} onChange={e=>setPass(e.target.value)} placeholder="senha" type="password"
          onKeyDown={e=>e.key==="Enter"&&submit()}
          style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`1px solid ${C.border}`,
            background:C.bgEl,color:C.cream,fontSize:13,outline:"none",marginBottom:10,fontFamily:"inherit"}}/>}
        <button onClick={submit} disabled={load} style={{width:"100%",padding:"13px 0",borderRadius:10,
          border:"none",background:load?C.border:C.rust,color:load?C.muted:"#fff",
          fontSize:13,fontWeight:600,cursor:load?"not-allowed":"pointer"}}>
          {load?"Aguarde...":(mode==="magic"?"Enviar link de acesso":"Entrar")}
        </button>
        {msg&&<div style={{marginTop:12,fontSize:12,color:msg.startsWith("✓")?C.greenL:C.rustL,textAlign:"center"}}>{msg}</div>}
      </div>
    </div>
  );
}
...
function EssencialCard({task,value,onChange}){
  const p=PILLARS.find(x=>x.id===task.pillar); const done=value===true;
  return (
...
function MultiCard({task,value,onChange}){
  const p=PILLARS.find(x=>x.id===task.pillar); const isAlim=task.id==="alimentacao";
  return (
...
function ConfigScreen({ taskConfig, scoreLabels, onSave, onBack }) {
  ...
  const setPoints = (id, val) => {
    ...
    setCfg(p=>({...p,[id]:{...p[id],points:num}}));
  };
  const setLevelPts = (id, idx, val) => {
    const num = parseInt(val); if (isNaN(num)||num<0) return;
    setCfg(p=>{
      const pts = Array.isArray(p[id].points) ? [...p[id].points] : [0,p[id].points];
      pts[idx] = num;
      return {...p,[id]:{...p[id],points:pts}};
    });
  };

  const handleSave = async () => { setSaving(true); await onSave(cfg, labels); setSaving(false); };
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",color:C.cream,maxWidth:430,margin:"0 auto"}}>
      ...
      <div style={{padding:"20px 20px 12px",background:`linear-gradient(180deg,${C.bgCard} 0%,transparent 100%)`,
        ...
        <button onClick={onBack} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer",lineHeight:1}}>←</button>
        <div>
          <div style={{fontSize:16,fontFamily:"'Playfair Display',serif",fontWeight:700,color:C.cream}}>Dia Ideal</div>
          <div style={{fontSize:10,color:C.muted}}>Configure pesos e ações ativas</div>
        </div>
      </div>
      ...
      <div style={{padding:"0 20px 100px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:isArr?6:0}}>
                      ...
                      <div onClick={()=>setActive(t.id,!(tc.active!==false))}
                        style={{width:32,height:18,borderRadius:9,
                          background:tc.active!==false?C.rust:C.border,
                      ...
                      {!isArr&&(
                        <div style={{display:"flex",alignItems:"center",gap:4}}>
                          ...
                          <input type="number" min="0" max="50" value={pts||0}
                        ...
                        {t.levels.map((lv,i)=>(
                          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                            ...
                            <input type="number" min="0" max="50"
                              value={Array.isArray(pts)?pts[i]:0}
        ...
        <button onClick={handleSave} disabled={saving} style={{width:"100%",padding:"14px 0",borderRadius:12,
          border:"none",background:saving?C.border:C.rust,color:saving?C.muted:"#fff",
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      let u = null;
      // 1. Voltou de magic link? Captura token do hash da URL
      if (window.location.hash.includes("access_token")) {
        u = await sb.handleMagicLinkCallback();
      }
      // 2. Tenta sessão salva — se expirada, faz refresh silencioso
      if (!u) u = await sb.restoreOrRefresh();
      if (u) { setUser(u); await loadData(u.id); }
      setAppReady(true);
    })();
  },[]);

  useEffect(()=>{
    if(!user||!gcal.isConfigured())return;
  ...
  const loadData=async userId=>{
    const [{data:day},{data:hist},{data:prof}]=await Promise.all([
      sb.getDay(userId,key),
      sb.getDays(userId,30),
      sb.getProfile(userId),
    ]);
  ...
  const persist=useCallback(patch=>{
    ...
    saveTimer.current=setTimeout(async()=>{
      setSyncing(true);
      await sb.upsertDay(user.id,key,patch);
      setSyncing(false);
    },800);
  },[user,key]);

  const saveConfig=useCallback(async(newTaskCfg,newLabels)=>{
    setTaskConfig(newTaskCfg); setScoreLabels(newLabels);
    if(user) await sb.upsertProfile(user.id,{task_config:newTaskCfg,score_labels:newLabels});
    setShowCfg(false);
  },[user]);
  ...
  if(!user) return <AuthScreen onAuth={async u=>{setUser(u);await loadData(u.id);}}/>;
      ...
      <div style={{padding:"20px 20px 0",background:`linear-gradient(180deg,${C.bgCard} 0%,transparent 100%)`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <button onClick={()=>setShowCfg(true)} style={{background:"none",border:`1px solid ${C.border}`,
              borderRadius:8,padding:"5px 8px",color:C.muted,fontSize:12,cursor:"pointer"}}>⚙</button>
          </div>
        </div>
        <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,marginTop:16}}>
          {[["hoje","Hoje"],["pilares","Pilares"],["hist","Histórico"]].map(([k,l])=>(
            <button key={k} onClick={()=>setTab(k)} style={{flex:1,padding:"10px 0",fontSize:12,fontWeight:500,
              background:"transparent",border:"none",borderBottom:`2px solid ${tab===k?C.rust:"transparent"}`,
              color:tab===k?C.cream:C.muted,cursor:"pointer",transition:"all 0.2s",marginBottom:-1}}>{l}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:"16px 20px 100px"}}>
        ...
        {tab==="hoje"&&<>
              ...
              <div style={{background:`${prof.color}15`,border:`1px solid ${prof.color}40`,
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:12,color:C.cream,fontWeight:500}}>
                    Calendário: <strong style={{color:prof.color}}>{prof.label}</strong>
                  </div>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",padding:"4px 0",marginBottom:14}}>
            {Object.entries(PROFILES).map(([k,p])=>{
              const active=today.profile===k; const isD=today.gcal_detected===k;
              return (
          ...
          {PILLARS.map(p=>{
            ...
            return (
              <div key={p.id} style={{background:C.bgCard,borderRadius:14,padding:14,border:`1px solid ${C.border}`,marginBottom:10,borderLeft:`3px solid ${p.color}`}}>
                ...
                {pT.map(t=>{
                  const v=today.checks[t.id]; const done=v&&v!==false;
                  return (
        ...
        {tab==="hist"&&<>
          <div style={{background:C.bgCard,borderRadius:16,padding:16,border:`1px solid ${C.border}`,marginBottom:14}}>
            ...
            {last7.length<2?(
              ...
              <div style={{display:"flex",alignItems:"flex-end",gap:8,height:84}}>
                {last7.map(([date,data])=>{
                  const s=data.score||0; const h=Math.max(4,(s/100)*56); const {color}=scoreMeta(s,scoreLabels);
          ...
          {history.slice(0,14).map(d=>{
            const s=d.score||0; const {label,color}=scoreMeta(s,scoreLabels); const prof=PROFILES[d.profile];
            return (
